...
| `src/drawer.ts` | 设置面板：**面向玩家**——6 条词化控件（灵敏度 / 雨量 / 烟花大小 / 密度 / 弹跳）+ 4 个配色预设 + 「雨落在身后」开关；「高级设置」折叠区里才是碰撞体、调试、导出配置 |
...
**两根进度条的语义**：永远表示「触发进度」，触发之后变成状态标签（「下雨中」「烟花中」）。
...
- 雨：尺寸系数 `rainK = clamp(短边/820, 0.45, 1)` 只乘线宽与长度（线宽下限 1.2 CSS 像素、
  长度下限 10 px）；运动系数 `rainMoveK = clamp(短边/820, 0.7, 1)` 乘速度与漂移。
  两者**不共用**——一起压就是牙签。短边 < 500 时三层 alpha ×1.2（封顶 0.95）。
