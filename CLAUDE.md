...
浏览器端 AR 表情特效：微笑 → 全屏下雨，大笑 → 从嘴部炸开烟花，烟花粒子与头部椭圆物理碰撞。
...
| `src/particles.ts` | Float32Array 粒子池、雨/烟花发射器、椭圆碰撞、发光贴图 |
| `src/hud.ts` | 开始页、引导、状态提示、手动触发、错误页、调试面板 |
...
`head` = landmark 10/152/234/454 拟合的椭圆（像素坐标，已镜像），渲染帧之间指数平滑。
...
**爆发不是状态，是 Laughing 内部的事件**：进入即大爆发 → 冷却 1.2 s → 冷却内每 0.6 s 补一发小烟花。
离开 Laughing 后 1.5 s 内继续稀疏补发（情绪残留）。

强度映射：雨量 = smoothstep(smileExit, 0.80, smile)，缓入 0.4 s / 缓出 0.8 s；
...
L2  #hud      DOM：引导文字+进度条（底部居中）、手动触发圆钮（右下 56px）、齿轮（右上）
L3  抽屉      DOM，默认隐藏
```
...
雨色 `#BFD4F2`，三层景深透明度 0.25/0.45/0.7，速度 300/480/720 px/s。

## 碰撞反馈分层

- 雨滴撞头：溅成 2–3 颗小水花后消失，**无闪光**，每帧最多 2 次溅射（持续性交互要克制）。
- 烟花撞头：沿椭圆法线反弹（弹性 `restitution`，速度衰减 0.85）+ 该粒子闪白 50 ms
  + 头部椭圆边缘微光脉冲 100 ms（200 ms 内不重复）。

## URL 参数

`?debug=1` 调试面板 + 键盘 1/2 触发 · `?mode=pro` 打开抽屉 · `?mode=clean` 隐藏齿轮与引导
· `?cfg=<base64 json>` 覆盖配置

## 边界情况

权限拒绝 → 错误页（含设置路径 + 「先用手动模式体验」）；无脸 3 s → 提示；
脸太小/置信度低 → 光线提示；页面隐藏 → 暂停循环并重置时间戳；resize/横竖屏 → 重算画布。
**无摄像头也必须能玩**：右下角圆钮点一下下雨、长按放烟花。这也是评审的后门。
