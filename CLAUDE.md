...
`head` = landmark 10/152/234/454 拟合的**带滚转角的椭圆** `{cx, cy, rx, ry, rot}`
（像素坐标，已镜像），渲染帧之间指数平滑，角度插值走最短弧。
两个必须保留的处理：**椭圆跟随头部滚转**（碰撞在头部局部坐标系里做），
以及**沿头部朝上方向外扩 33% 脸高**——468 个关键点最高只到额头，颅顶和头发不在关键点里。
...
L2  #hud      DOM：引导文字+进度条（底部居中，文字走完淡出但**进度条常驻**）、手动触发圆钮（右下 56px）、齿轮（右上）
L3  抽屉      DOM，默认隐藏
```
