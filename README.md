...
**[Live Demo → smile.kkyq.hk.cn](https://smile.kkyq.hk.cn)**（主访问地址；备用地址 [smile-rain-fireworks.vercel.app](https://smile-rain-fireworks.vercel.app)，若开启 Vercel 部署保护则需登录，真机请用自定义域名）
...
界面上的微笑条画的是幅度 `smileAmp = clamp(smile / smileEnter, 0, 1)`，随当前笑容涨落，满格不代表已触发下雨；
进入 Smiling 仍要求 smile ≥ `smileEnter` 持续 600 ms，这段候选计时只供状态机用，**不驱动幅度条**。
大笑条画的是 `laughProgress = min(1, jawOpen / laughJaw, smile / 微笑退出)`，任何时候都按当前信号显示：进入 Laughing 后满格，嘴一合就回落，不等 400 ms 退出防抖。
大笑期间微笑条归零、整行变淡；变淡跟的是实时信号（张嘴且在笑，带 0.6× 退出迟滞），嘴一合当帧亮回来。退出后按当前信号显示，不保留历史幅度。
...
Laughing 期间雨量目标为 0，雨约 0.4 s 淡出。

离开 Laughing 后还有 1.5 s 的**情绪残留**——烟花逐渐稀疏而不是硬停，像笑完之后还有余韵。余韵独立衰减，不阻塞随后的微笑响应；切换期间画面里可能短暂同时有雨和尚未消散的烟花。
...
- **椭圆模式**（手动关闭分割、低档自动关闭分割、或 Worker 尚未给出可用遮罩）：烟花粒子相对带滚转角的椭圆做几何判定。
...
**谁付钱。** 无服务端，检测和渲染都在浏览器里。若接入直播并在主播端把特效合成进推流画面，表情与分割模型只需在主播端跑一份，观众端不必运行这些模型，平台也不必为它们付服务端推理费（本原型尚未接入推流链路，托管、分发和直播本身的成本另计）。首次点「开始」后要加载 wasm 3.3 MB + 表情模型 3.3 MB + 分割模型 0.2 MB（Brotli 传输体积，线上实测；解压后约 16 MB），后续可复用浏览器缓存，缓存失效或资源更新时需重新下载。所以这笔下载放在点击之后并带进度条，而不是压在首屏。

**主播端的算力预算。** 直播间里推流、美颜、连麦已经在跑，特效只能用剩下的。所以表情检测约 20 Hz 而不是每帧；人像分割按档位每 4 或 6 次 rAF 回调调度一次（约 60 Hz 回调下名义 10–15 Hz，Worker 忙时跳过）、低档自动关；60 s 无人时全部停掉。目标是在不明显影响推流与基础互动的前提下运行特效；实际影响仍需在直播环境里验证。
...
**生产成本。** 截至 2026-09-15，这个原型累计 119 个 commit、约 5,500 行 TS/CSS，由多个 AI 分工（Claude 定义与架构、Cursor 实现、Claude Design 界面、其它模型做调研），人工负责定义、验收和纠偏（`docs/vibe-log.md` 记了 7 次）。对「自动化生产工具」这件事来说，这笔账和 Demo 本身一样重要：AI 减少了实现与沟通的工作量，但定义和验收仍由人来承担。
...
| `person … ms` | 人像分割耗时 | Worker 里最近一次 `segmentForVideo`，按档位每 4 或 6 次 rAF 回调调度一次，Worker 忙时跳过 |
...
最大的坑在微笑和大笑的衔接。AI 第一版给两个阈值、各管一个开关，看起来能跑，一上镜就露馅：突然大笑会先下雨再放烟花，笑到临界处雨反复开关，说话张嘴也会炸。它把「有数值」当成了「有体验」。我先给它定规矩，再一轮轮对着镜头调数：「表情是三个状态：待机、微笑、大笑。进入门槛高于退出，不许在临界值来回跳；切换前要稳住一小段时间；待机可以直接进大笑。数值先给默认，我来试。」真机上突然大笑不再先下雨，说话也不再误触。
