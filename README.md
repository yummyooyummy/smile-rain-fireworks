...
碰撞体在抖。AI修了几何——椭圆改成随头旋转、覆盖颅顶——仍抖。根因被漏掉：4个landmark每帧在跳，抖动1:1传进碰撞体；固定系数平滑只能二选一：静止抖或运动拖。纠偏：36点PCA拟合把抖动平均掉；One Euro滤波截止频率随速度自适应，静止重滤、运动低延迟，只滤5个标量。实测：角度误差<0.5°，抖动压到23%，滞后17ms。教训：AI代码「看起来对」和「可验证地对」的差距只有测试能填。

> 完整记录见 `docs/vibe-log.md`（5 条，含纠偏 prompt 原文）。
