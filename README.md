...
大笑条是 `laughProgress`（张嘴与微笑退出两个条件的 gate；进入 Laughing 后满格）。
...
（`npm run verify:head`）：用**合成的**歪脸检查角度还原误差、椭圆的包围性、以及滤波指标。
它当场抓到一个目测发现不了的问题——只外扩顶部并上移中心，椭圆会在**下侧翼**收进轮廓内约 0.3%，
于是补了一个 2% 的安全裕度。脚本通过不等于真机验收通过。

| 验证项（合成输入，`npm run verify:head`） | 脚本记录（含随机噪声，与 `docs/vibe-log.md` 若有出入不强行抹平） |
| --- | --- |
| 头部朝向还原（±60°） | 无噪声时约 0°；带点噪声时曾测到最大约 0.5°（脚本通过门槛 2.5°；日志里写过 < 0.5°） |
| 椭圆包围整圈轮廓 | 加安全裕度后脚本通过 |
| One Euro 静止去抖 | 输出逐帧变化约为输入的 22%，即**降至原来的约 22%**（不是「降低 22%」）。日志里也曾写作约 23% |
...
| **wasm 与模型自托管** | 评审很可能在国内网络下打开，官方 `storage.googleapis.com` 等地址经常不可达。运行时先用同源资源，CDN 只作兜底 |
...
| 面板字段 | 名称 | 实际含义 |
| --- | --- | --- |
| `fps` | 回调耗时倒数（非实际 FPS） | `1000 /` 回调耗时 EMA |
| `frame` | 回调耗时 EMA | 本帧 rAF 回调执行耗时的 EMA |
...
提示本身很容易写，但它要求 `numFaces ≥ 2`。MediaPipe 在 `numFaces === 1` 时才对关键点做官方平滑；
放开之后抖动和推理开销都可能上升，这个 Demo 里为了一句提示去换主路径稳定性，不划算。
所以保持 `numFaces = 1`，写成已知限制，而不是假装它不存在。
...
分享入口（`#shareBtn`）对用户隐藏。画面合成 `composeFrame` 仍留在代码里，但当前没有独立可见的截图按钮，用户无法从界面触发截图或分享。
...
碰撞体抖动时，AI 首轮只修正了椭圆朝向与颅顶覆盖，遗漏了关键点噪声和固定平滑的滞后问题。我要求从信号来源排查：用整圈 36 点 PCA 拟合减少单点噪声影响，再用 One Euro 滤波兼顾静止稳定与运动响应。数值脚本验证了角度、包围性与滤波表现，但不能替代真机验收。教训：不要把“代码看起来合理”当作正确证据，应拆解假设，用可复现测试验证，再检查实际体验。

完整记录见 `docs/vibe-log.md`。
