...
最大陷阱是「碰撞体抖动」：AI 首轮只修看得见的几何（椭圆朝向、颅顶覆盖），漏了根因——关键点本身在抖，固定平滑只能在稳和快之间二选一。纠偏 prompt：**「头部碰撞体必须：1) 用 face oval 36 点 PCA 拟合，不用 4 个点；2) 椭圆随头旋转；3) 参数经 One Euro 滤波并在检测帧间插值，禁止把原始 landmark 直接喂给碰撞判定。」** 再用脚本验证角度、包围性、滤波。教训：AI 只修症状，根因要自己追。

完整记录见 `docs/vibe-log.md`（7 条，含 prompt 原文与前后数据）。
