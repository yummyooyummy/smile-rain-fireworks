...
浏览器端 AR 表情互动原型：**微笑时全屏下雨，大笑时从嘴部炸开烟花，烟花粒子与头部产生物理碰撞。**
...
| 类型 | 反馈 | 频率控制 |
| --- | --- | --- |
| 雨滴撞头 | 溅成 2–3 颗小水花后消失，**无闪光** | 每帧最多 2 次溅射 |
| 烟花撞头 | 沿椭圆法线反弹 + 该粒子闪白 50 ms + 头部边缘微光脉冲 100 ms | 脉冲 200 ms 内不重复 |
...
调试参数：`?debug=1` 调试面板与键盘 1/2 触发 · `?mode=pro` 打开调参抽屉 ·
